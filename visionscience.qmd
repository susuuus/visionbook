# Looking at Images {#chap:looking_at_images}
...
images and to pay attention to individual pixels. The image in is a tiny
image with just $32 \times 32$ pixels. It is so small that we can
actually look at each single pixel and think about how it is that we can
make sense of it. For instance, let's look at the pixel in row 21
counting from the top, and column 20 counting from the left. That pixel
corresponds to a dark gray pixel, which is near a couple of other pixels
with similar intensity, surrounded by a set of white pixels. That pixel
seems to be a pen, but how do we know what it is? The pen is barely
three pixels long and one pixel wide. How can that provide enough
information?
...
Photoshop.](figures/visionscience/DALLE_32x32.png){#fig:pexels-retha-ferguson
width=".8\\linewidth"}
...
<figure>
<p><img src="figures/visionscience/dalle_hand.png" alt="image" />   <img
src="figures/visionscience/dalle_mug.png" alt="image" />   <img
src="figures/visionscience/dalle_pen.png" alt="image" /></p>
<figcaption>Small image patches from in isolation. It is difficult to
recognize image patches outside their natural context.</figcaption>
</figure>
...
shown in and try to write down everything you see down to the smallest
detail.

![A street in Paris. Is everything fine in this
picture?](figures/visionscience/ADE_train_00015819_messed_small.jpg){#fig:street_in_paris
width=".98\\linewidth"}
...
instance, are you sure you can make sense of all the chair legs in ? In
fact, this image has been manipulated so that some of legs do not
correspond to any chair.
...
contains thousands of tiny images within it (). A single photo can be
*big data*; if you chop up this photo you get a big dataset of tiny
images. A common trick in computer vision is to take a method that was
developed for processing a dataset and instead apply it to the set of
patches in an image, or vice versa.
...
The following sequence (), painted by Agata Lapedriza, shows different
...
<figure id="fig:agata_painting">
<p> 1<img src="figures/visionscience/fresa_1.jpg" alt="image" /> 2<img
src="figures/visionscience/fresa_2.jpg" alt="image" /> 3<img
src="figures/visionscience/fresa_3.jpg" alt="image" /> 4<img
src="figures/visionscience/fresa_4.jpg" alt="image" /></p>
<p>  5<img src="figures/visionscience/fresa_5.jpg" alt="image" /> 6<img
src="figures/visionscience/fresa_6.jpg" alt="image" /> 7<img
src="figures/visionscience/fresa_7.jpg" alt="image" /> 8<img
src="figures/visionscience/fresa_8.jpg" alt="image" /></p>
<figcaption>The following sequence shows different steps in the process
of painting a strawberry. <span><em>Source</em></span>: Original images
by Agata Lapedriza.</figcaption>
</figure>

## Tree Shadows and Image Formation

When walking under the shadow of a tree on a bright sunny day when the
sun is right above us (\[left\]), we can see how some light rays get
through the holes left between leaves, creating spots on the floor
(\[right\]).

<figure id="fig:tree_pinholes">
<p><img src="figures/visionscience/IMG_2842_tree_pinholes.jpg"
alt="image" />   <img
src="figures/visionscience/IMG_2837_tree_shadow.JPG" alt="image" /></p>
<figcaption>The shadow of this tree on a normal day projects dozens of
pictures of the sun on the ground.</figcaption>
</figure>
...
irregular profiles (\[left\]). Instead, as shown in (right), the spots
appear to be circular and of similar size. This observation already
puzzled Aristotle. What could be happening? As a hint, shows the shadow
...
Look at the two drawings in . The lines in the drawing on the left
correspond to tiles in a floor, the line drawing on the right is the
same drawing rotated 90 degrees. Which one seems to be an horizontal
surface and which one seems vertical? Why? The differences in the line
orientations are subtle and yet it produces a very different perception.
...
surface.](figures/visionscience/vertical_or_horizontal.eps){#fig:vertical_or_horizontal
width="1\\linewidth"}
...
(). Blur is due to the fact that the camera sensor is collecting light
during a period of time while the car is moving and the picture that
results is the average of several translated copies of the same image.
However, note that not all of the objects are equally blurry. Things
that are far away might still look sharp while things nearby appear very
blurry.
...
We learned about the generic view principle in , and this idea comes up
a lot in computer vision. But it turns out that in most photographs it's
easy to find accidents. This is because photos contain so many things,
thousands of tiny images, and there are so many coincidences that could
occur. shows a famous photo and we will look for some coincidences in
it. The coincidences are of a specific variety: generically, if you see
smooth, continuous line segments you assume that they lie on a single
object because it's quite a coincidence for two objects to line up just
so their contours perfectly align. The gestalt psychologists called this
the **principle of continuity**. The insets for show several locations
in the photo "Migrant Mother" by Dorothea Lange where exactly this
...
cue that the ball is above the surface in figures
[1.10](#fig:cues_for_suppport){reference-type="ref"
reference="fig:cues_for_suppport"}(a) and
[1.10](#fig:cues_for_suppport){reference-type="ref"
reference="fig:cues_for_suppport"}(c), while the slack string provides
evidence for the ball being on the surface in figures
[1.10](#fig:cues_for_suppport){reference-type="ref"
reference="fig:cues_for_suppport"}(b) and
[1.10](#fig:cues_for_suppport){reference-type="ref"
reference="fig:cues_for_suppport"}(c). Only is a doctored photograph.

In which of these figures does the ball appear to be in contact with the
ground plane?

<figure id="fig:cues_for_suppport">

<figcaption>Cues for support. The relative location between an object,
and its shadow on the ground is a powerful cue for contact.</figcaption>
</figure>

## Looking at Raindrops

When it rains you get to see a rare sight. Every raindrop refracts a
panorama of the scene around it (). What you are seeing is not just one
photo but hundreds of images of the scene, one in each raindrop, all
from slightly different angles. It turns out specially designed cameras
have been made that do the same thing, and they are called **light field
cameras**. These cameras use an array of tiny lenses rather than
raindrops, but the result is similar: they capture what the scene looks
like from hundreds of different viewing angles, and from that density of
information they can achieve many interesting tricks, such as
triangulating the depth of objects in the scene and synthetically
...
left.](figures/visionscience/droplets.jpg){#fig:droplets
width="1.0\\linewidth"}
...
without touching it, as shown in .
