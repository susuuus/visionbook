...
- $a = p(X = \mathbf{x} \given \ldots)$ is the probability of the
  realization $X = \mathbf{x}$, possibly conditioned on some
  observations ($a$ is a scalar).

- $f = p(X \given \ldots)$ is the probability distribution over $X$,
...
- $p(\mathbf{x} \given \ldots)$ is shorthand for
  $p(X = \mathbf{x} \given \ldots)$. and so forth, following these
  patterns.
