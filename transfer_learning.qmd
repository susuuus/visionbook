# Transfer Learning and Adaptation {#chapter-transfer_learning}

## Introduction

A common criticism of deep learning systems is that they are data
hungry. To learn a new concept (e.g., "is this a dog?") may require
showing a deep net thousands of labeled examples of that concept.
Conversely, humans can learn to recognize a new kind of animal after
seeing it just once, an ability known as .
...
![](./figures/transfer_learning/pretraining_and_adaptation.png){width="85%"}
...
ways. Then, in section
[1.9](#sec-transfer_learning:combinatorial_catalog){reference-type="ref"
reference="sec-transfer_learning:combinatorial_catalog"}, we will
...
[\[alg:transfer_learning:finetuning\]](#alg:transfer_learning:finetuning){reference-type="ref"
reference="alg:transfer_learning:finetuning"}, with $f$ and $f^{\prime}$
indicated as just a continually learning $f_{\theta}$ with different
iterates of $\theta$ as learning progresses.

::: algorithm
[]{#alg:transfer_learning:finetuning
label="alg:transfer_learning:finetuning"} **Input:** initial parameter
vector $\theta^{0}$, data $\{x^{(i)},y^{(i)}\}_{i=1}^N$,
$\{x^{\prime^{(i)}},y^{\prime^{(i)}}\}_{i=1}^M$, learning rates $\eta_1$
and $\eta_2$ **Output:** trained models $f_{\theta^{N}}$ and
$f_{\theta^{N+M}}$ **Pretraining:**
...
section
[\[sec-perceptual_organization:affinity_based_segmentation\]](#sec:perceptual_organization:affinity_based_segmentation){reference-type="ref"
reference="sec-perceptual_organization:affinity_based_segmentation"}).
...
![](./figures/transfer_learning/finetuning_certain_modules_example.png){width="70%"}
...
reference="chapter-intro_to_learning"}, we described it as *learning
...
reference="sec-intro_to_learning:image_classification"}). The student, a
...
![](./figures/transfer_learning/KD_diagram.png){width="100%"}
...
vision-language models @radford2021learning (section
[\[sec-VLMs:CLIP\]](#sec:VLMs:CLIP){reference-type="ref"
reference="sec-VLMs:CLIP"}). Second, prompting can also be applied to
...
![](./figures/transfer_learning/prompting.png){width="35%"}
...
![](./figures/transfer_learning/vp_hyojin.png){width="100%"}
...
adversarial noise we covered in section
[\[sec-bias_and_shift:adversarial_attacks\]](#sec:bias_and_shift:adversarial_attacks){reference-type="ref"
reference="sec-bias_and_shift:adversarial_attacks"}.  However,
here we are not adding noise that *hurts* performance but instead adding
...
reference="chapter-transformers"} for details on transformers). This new
...
reference="chapter-conditional_generative_models"}) to train systems can
...
reference="chapter-generative_models"}) that translates the data in one
...
[\[alg:transfer_learning:domain_adaptation_by_translation\]](#alg:transfer_learning:domain_adaptation_by_translation){reference-type="ref"
reference="alg:transfer_learning:domain_adaptation_by_translation"}.
...
**Input:** training data
$\{x_{\texttt{train}}^{(i)},y_{\texttt{train}}^{(i)}\}_{i=1}^N$, test
data, $\{x_{\texttt{test}}^{\prime^{(i)}}\}_{i=1}^M$ **Output:** trained
model $F$ **Train predictor on `train`:**
$f = \mathop{\mathrm{arg\,min}}_{f} \mathbb{E}_{x_{\texttt{train}},y_{\texttt{train}}}[\mathcal{L}(f(x_{\texttt{train}}),y_{\texttt{train}})]$
**Train translator $\mathcal{X}_{\texttt{test}}$ to
$\mathcal{X}_{\texttt{train}}$:**
$G = \mathop{\mathrm{arg\,min}}_g \max_d \mathbb{E}_{x_{\texttt{train}}}[\log d(x_{\texttt{train}}))] + \mathbb{E}_{x_{\texttt{test}}}[\log(1-d(g(x_{\texttt{test}}))]$
return $F = f \circ g$
:::
...
[\[chapter-generative_models\]](#chapter:generative_models){reference-type="ref"
reference="chapter-generative_models"}) produce synthetic data that
...
[\[chapter-generative_models\]](#chapter:generative_models){reference-type="ref"
reference="chapter-generative_models"}). Generative models can also
...
[\[chapter-generative_modeling_and_representation_learning\]](#chapter:generative_modeling_and_representation_learning){reference-type="ref"
...
reference="tab:generative_models:types_of_gen_model"}, we present the
...
and/or during adaptation.
  ...
  Method              Learning                                                                                                                                                                                                                                                              Inference
                      [pretrained]{style="color: pretrained_color"} / [adapted]{style="color: adapted_color"} / [both]{style="color: both_color"}                                                                                                                                           
  Generative data     $\mathop{\mathrm{arg\,min}}_{\textcolor{adapted_color}{\theta}} \mathbb{E}_{\textcolor{pretrained_color}{x},\textcolor{pretrained_color}{y}}[\mathcal{L}(f_{\textcolor{adapted_color}{\theta}}(\textcolor{pretrained_color}{x}), \textcolor{pretrained_color}{y})]$   $f_{\textcolor{adapted_color}{\theta}}(x)$
  Distillation        $\mathop{\mathrm{arg\,min}}_{\textcolor{adapted_color}{\theta}} \mathbb{E}_{x}[\mathcal{L}(f_{\textcolor{adapted_color}{\theta}}(x), \textcolor{pretrained_color}{t}(x))]$                                                                                            $f_{\textcolor{adapted_color}{\theta}}(x)$
  Prompting           $\mathop{\mathrm{arg\,min}}_{\textcolor{adapted_color}{\epsilon}} \mathbb{E}_{x,y}[\mathcal{L}(f_{\textcolor{pretrained_color}{\theta}}(x \oplus \textcolor{adapted_color}{\epsilon}), y)]$                                                                           $f_{\textcolor{pretrained_color}{\theta}}(x \oplus \textcolor{adapted_color}{\epsilon})$
  Finetuning          $\mathop{\mathrm{arg\,min}}_{\textcolor{both_color}{\theta}} \mathbb{E}_{x,y}[\mathcal{L}(f_{\textcolor{both_color}{\theta}}(x), y)]$                                                                                                                                 $f_{\textcolor{both_color}{\theta}}(x)$
  Probes / heads      $\mathop{\mathrm{arg\,min}}_{\textcolor{adapted_color}{\phi}} \mathbb{E}_{x,y}[\mathcal{L}(h_{\textcolor{adapted_color}{\phi}}(f_{\textcolor{pretrained_color}{\theta}}(x)), y)]$                                                                                     $h_{\textcolor{adapted_color}{\phi}}(f_{\textcolor{pretrained_color}{\theta}}(x))$
  Learned loss        $\mathop{\mathrm{arg\,min}}_{\textcolor{adapted_color}{\theta}} \mathbb{E}_{x,y}[\textcolor{pretrained_color}{\mathcal{L}}(f_{\textcolor{adapted_color}{\theta}}(x), y)]$                                                                                             $f_{\textcolor{adapted_color}{\theta}}(x)$
  Learned optimizer   $\textcolor{pretrained_color}{\mathop{\mathrm{arg\,min}}}_{\textcolor{adapted_color}{\theta}} \mathbb{E}_{x,y}[\mathcal{L}(f_{\textcolor{adapted_color}{\theta}}(x), y)]$                                                                                             $f_{\textcolor{adapted_color}{\theta}}(x)$
  ------------------- --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- ------------------------------------------------------------------------------------------
