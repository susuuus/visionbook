...
![Prompting a transformer with a learnable input token. The prompt token is mixed into the network via attention; therefore \textit{no} parameters of the original network have to be updated (see @sec-transformers).](./figures/transformers/prompting_a_transformer.png){width="40%" #fig-transformers-prompting_a_transformer}
...
![Unpaired domain adaptation via adversarial translation.](./figures/transfer_learning/domain_adaptation_by_translation.png){width="100%" #alg-transfer_learning-domain_adaptation_by_translation}
