...
![3D reconstruction using the simple visual system from @sec-simplesystem](figures/simplesystem/views.png){width="100%" #fig-chatgpt_blockskworld}
...
images. @fig-chatgpt_blockskworld shows the result of asking GPT4-v to
...
![(a) Input image and text prompt to ChatGPT. (b) Output by ChatGPT (Gpt4-v, version from Nov 21, 2023) when asked to describe the image from the simple world.](figures/simplesystem_revisited/chatgpt_blockskworld.png){width="100%" #fig-chatgpt_blockskworld}

But it's not all solved. Because if we reconstruct the image from this
text, it looks not quite right: @fig-chatgpt_blockskworld2. What is
...
![Output by ChatGPT when asked to render a new image with the same elements as in @fig-chatgpt_blockskworld This conversation is a continuation of the same chat from @fig-chatgpt_blockskworld](figures/simplesystem_revisited/chatgpt_blockskworld_2.png){width="50%" #fig-chatgpt_blockskworld2}
