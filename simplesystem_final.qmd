...
visual world that we introduced in chapter
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}.
...
connecting these building blocs to produce astonishing results. However,
...
chapter
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"} with a hand-crafted approach by
...
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}, our goal was to recover the
...
![Images and depth maps from the training set. {\em Source](figures/simplesystem_revisited/examples_training_midas.jpg){width="100%" #fig-examples_training_midas}
...
[\[sec-camera_parameters\]](#sec-camera_parameters){reference-type="ref"
reference="sec-camera_parameters"}. We will need to know the intrinsic
...
reference frame as, $$\begin{aligned}
\begin{bmatrix}
X \\
Y \\ 
Z 
...
\end{aligned}$${#eq-recoverworldcoord}
...
coordinates: $$\mathbf{K} = \mathbf{M} \mathbf{R}$$
...
\end{bmatrix}.
\end{aligned}$$
![](figures/simplesystem_revisited/projection-revisted.png){width=".24"}
In order to apply equation (@eq-recoverworldcoord), we need to define
...
Now we are ready to use equation (@eq-recoverworldcoord) and the output
of the system.

## From 2D Images to 3D

Let's start running the model on the same input image that we used as a
running example in chapter
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}. We just need to apply the neural
...
edges or any of the other features that we computed in chapter
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}. Instead, the neural network has
...
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}, to show that the algorithm for 3D
interpretation gives reasonable results, we can re-render the inferred
3D structure from different viewpoints.

![3D reconstruction using a pretrained model. The images show multiple view points of the reconstructed 3D scene from \fig{\ref{fig:simplesystem_revisited_img1](figures/simplesystem_revisited/views_midas.jpg){width="100%" #fig-views_midas}
...
algorithm we defined in chapter
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}:

![3D reconstruction using the simple visual system from \chap{\ref{chapter:simplesystem](figures/simplesystem/views.png){width="100%" #fig-chatgpt_blockworld}
...
![](figures/simplesystem_revisited/comparison_midas_simpleworld.jpg){width="100%"}
...
autoregressive modeling objective (chapter
[\[chapter-transformers\]](#sec-transformers){reference-type="ref"
reference="chapter-transformers"}). On another level, you can understand
...
![](figures/simplesystem_revisited/chatgpt_blockworld.png){width="100%"}
...
![Output by ChatGPT when asked to render a new image with the same elements as in \fig{\ref{fig:chatgpt_blockworld](figures/simplesystem_revisited/chatgpt_blockworld_2.png){width=".5" #fig-chatgpt_blockworld2}
...
in chapter
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}, but it works in situations where our
...
[\[chapter-simplesystem\]](#sec-simplesystem){reference-type="ref"
reference="chapter-simplesystem"}, we had a comprehensive understanding
...
![](figures/simplesystem_revisited/gibson_bird.png){width=".06"}
