...
![(a) Stereo anaglyph of the Titanic \cite{McManus2022](figures/3d_scene_understanding/titanic.png){width="100%" #fig-titanic}
...
![Random dot stereogram. Both images are almost identical. The difference is that there is a square in the middle that is displaced by a few pixels left-right between the two images. When each image it seen by one eye you should see a square floating in from of a flat background. You can use a stereoscope or free fusion to see it. %\reviewcomment{When I free-fuse this pair, I see an irregularly shaped hole in the center, opening to a plane behind the rest of the image, not a square floating in front of a background.](figures/3d_scene_understanding/random_dot_stereogram.png){width="100%" #fig-random_dot_stereogram}
...
![Simple stereo. (a) Two cameras, of identical focal length, $f$, and separated by an offset, $T$, image the point $P$ onto the two-dimensional (2D) points $[x_L,y_L]^\mathsf{T}$ and $[x_R,y_R]^\mathsf{T}$ at each camera.  (b) The similarity of the two triangles in leads to @eq-depthFromDisparity](figures/3d_scene_understanding/triangularization_stereo.png){width="100%" #fig-stereo}
...
automatically is the crux of the stereo problem.
  ...
  --------------------------------------------------------
   ![Two cameras, displaced from each other laterally, photograph the same scene, resulting in images (a) and (b).  Colored rectangles show some identifiable features in image (a), and where those features appear in image (b).  Arrows show the corresponding displacements, which reveal depth through @eq-depthFromDisparity](figures/stereo/officeLeft.jpg){width="47.5%" #fig-stereomatch}
                           \(a\)
  --------------------------------------------------------
...
feature detector to the images of @fig-stereomatch.
  ...
  ------------------------------------------------------
   ![The feature-based stereo approach, illustrated for the stereo pair of @fig-stereomatch](figures/stereo/pointsLeft.jpg){width="31%" #fig-stereopoints}
                          \(a\)
  ------------------------------------------------------
...
![Geometry of a stereo pair and terminology. The figure shows the epipolar plane, epipolar lines, and epipoles ($e_1$ and $e_2$), for a given point $\mathbf{P](figures/3d_scene_understanding/epipolar_geometry.png){width="80%" #fig-epipolar_geometry_terminology}
