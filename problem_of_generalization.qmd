...
Polynomial regression is just like linear regression (chapter
[\[chapter-intro_to_learning\]](#sec-intro_to_learning){reference-type="ref"
reference="chapter-intro_to_learning"}) except that the hypothesis space
...
hyperparameter of the hypothesis space.Let us consider the setting where we use the least-squares ($L_2$) loss
...
$\mathbf{X}$ did in chapter
[\[chapter-intro_to_learning\]](#sec-intro_to_learning){reference-type="ref"
reference="chapter-intro_to_learning"}; in fact we often call matrices
of the feature representations of each datapoint also as a **data
matrix**. As an exercise, you can derive the closed form of the
...
:::{.column-margin}
When we get to
the chapters on neural nets we will see that data matrices appear all
over. A neural net is a sequence of transformations of an input data
matrix into increasingly more powerful feature representations of the
...
![](./figures/problem_of_generalization/under_and_overfitting.png){width="100%"}
...
minimize in empirical risk minimization (chapter
[\[chapter-intro_to_learning\]](#sec-intro_to_learning){reference-type="ref"
reference="chapter-intro_to_learning"}).
...
![](./figures/problem_of_generalization/under_and_overfitting_vs_polyK.png){width="48%"}
...
reference="chapter-neural_nets"}, can indeed either underfit or overfit,
...
overfit less than smaller nets. We discuss this further in section
[1.4](#sect:problem_of_generalization:rethinking_generalization){reference-type="ref"
reference="sect:problem_of_generalization:rethinking_generalization"}.
...
\end{aligned}$${#eq-problem_of_generalization} where $\lambda$ is a hyperparameter that controls the
strength of the regularization.

One of the most common regularizers is to penalize the $L_p$ norm of the
parameters of our model, $\theta$:
$$R(\theta) = \left\lVert\theta\right\rVert_{p}.$$  An
...
data, give preference to the simplest.### Regularizers as Probabilistic Priors
...
Remember the $\star$ problem from chapter
[\[chapter-intro_to_learning\]](#sec-intro_to_learning){reference-type="ref"
reference="chapter-intro_to_learning"}? $$\begin{aligned}
    3 \star 2 &= 36\nonumber \\
    7 \star 1 &= 49\nonumber \\
    5 \star 2 &= 100\nonumber \\
    2 \star 2 &= 16\nonumber
...
The first tool, *data*, was the topic of chapter
[\[chapter-intro_to_learning\]](#sec-intro_to_learning){reference-type="ref"
reference="chapter-intro_to_learning"}. In vision, the data is
...
![](./figures/problem_of_generalization/search_space_tools.png){width="65%"}
...
\end{aligned}$${#eq-problem_of_generalization} In
@fig-problem_of_generalization:more_data_more_constraints, bottom row,
...
![](./figures/problem_of_generalization/more_data_more_constraints.png){width="100%"}
...
![](./figures/problem_of_generalization/more_regularizer_more_constraints.png){width="100%"}
...
![](./figures/problem_of_generalization/fewer_hypotheses_more_constraints.png){width="100%"}
