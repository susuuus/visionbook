# Linear Image Filtering {#chapter-linear_image_filtering}
...
case of sampling the signal 30 times per second. As is common in signal
...
signal shown in @fig-contdiscsignala once every second
...
The signal in @fig-contdiscsignalb is a function that takes on the
...
values are scaled between 0 and 256): $$\begin{array}{cc}
\boldsymbol\ell= & 
\left[
...
![](figures/linear_image_filtering/whichoneislinear.png){width="100%"}
...
![](figures/linear_image_filtering/linear_filter-1.png){width=".4"}
...
![](figures/linear_image_filtering/space_of_functions-1.png){width=".25"}
...
![](figures/linear_image_filtering/fredo_birds1.jpg){width="100%"}
...
$$\ell_{\texttt{out}}\left[n,m \right] = \frac{1}{25} \sum_{k=-2}^{2} \sum_{l=-2}^{2} \ell_{\texttt{in}}\left[n+k,m+l \right]$$A system is an LTI system if it is linear and when we translate the
...
$${#eq-1dconv} The convolution computes the output values as a linear
...
so that: $$h \left[n\right] \circ \ell_{\texttt{in}}\left[n\right] =\sum_{k=-\infty}^{\infty} h \left[n-k \right] \ell_{\texttt{in}}\left[k \right] = \sum_{k'=-\infty}^{\infty} h \left[k' \right] \ell_{\texttt{in}}\left[n-k' \right]  = 
...
**convolutional layer** of a neural network as shown in
@fig-conv_filter2_nn.

![](figures/linear_image_filtering/conv_filter2-1.png){width=".5"}
...
$${#eq-2dconv} The next figure shows the result of a 2D convolution
...
![](figures/linear_image_filtering/circle_conv_example-1.png){width="100%"}
...
As shown in @fig-transformationsquizz2a, in the defocusing
...
translation invariant. As illustrated in @fig-transformationsquizz2b,
...
operation.
  ...
  ----------------------------------------------------------------------------------------------------
   ![](figures/linear_image_filtering/whyrotationisnotconvolution_a.png){width=".48"}
                                                 \(a\)
  ----------------------------------------------------------------------------------------------------

  ----------------------------------------------------------------------------------------------------
   ![](figures/linear_image_filtering/whyrotationisnotconvolution_b.png){width=".48"}
                                                 \(b\)
  ----------------------------------------------------------------------------------------------------
...
![](figures/linear_image_filtering/examples_cube2-1.png){width="100%"}
...
![](figures/linear_image_filtering/boundary-1.png){width="100%"}
...
such that $\ell(t) \circ \delta(t) = \ell(t)$. The impulse is defined as
...
strict sense) or the Dirac delta function.  The impulse has
...
reference="chapter-sampling"} when we discuss sampling.
...
![](figures/linear_image_filtering/correlation_vs_convolution-1.png){width="100%"}
...
![](figures/linear_image_filtering/normcorr-1.png){width="100%"}

Cross-correlation is a potential method for template matching, but it
has certain flaws. Consider matching the "a" template in @fig-normcorra
...
regardless of regions' content as shown in @fig-normcorrc.
...
as follows (@fig-normcorrd): $$\ell_{\texttt{out}}\left[n,m\right] 
                = 
                \frac{1}{\sigma \left[n,m \right]}
                \sum_{k,l=-N}^N
                \ell_{\texttt{in}}\left[n+k,m+l \right]
                \hat{h} \left[k,l \right]$$ Note that this equation is
similar to the correlation function, but the output is scaled by the
local standard deviation, $\sigma \left[m,n \right]$, of the image patch
...
@fig-normcorre shows the detected copies of the template
@fig-normcorra in the image @fig-normcorrb. The red bounding boxes in
@fig-normcorre correspond to the location with local maximum values of
the normalized cross-correlation (@fig-normcorrd) that are above a
...
![](figures/linear_image_filtering/impulse_response_room_a1-1.png){width=".6"}
...
![](figures/linear_image_filtering/impulse_response_room_a2-1.png){width="100%"}
...
![](figures/linear_image_filtering/impulse_response_room_b-1.png){width="100%"}
