...
[$\theta$]{style="background-color: comp_graph_param_bcolor"} to indicate *free* parameters, which are set via learning and are not the result of any other processing.
:::
...
![A generic layer in the computation graph. The braces represent the part of the computation graph we need to consider in order to evaluate $\mathbf{g}_{\texttt{out}}$, $\localgrad$, and $\costgradin$.](figures/backpropagation/generic_layer_g_L.png){#fig-backpropagation-generic_layer_g_L width="100%"}
...
:::{.column-margin}
The transpose is because, by convention, $\theta$ is a column vector while $\frac{\partial J}{\partial \theta}$ is a row vector; see the Notation section prior to chapter @sec-notation.
:::
...
![Backward operation of a neural net layer.](figures/backpropagation/mod_block_backward.png){#fig-backpropagation-mod_block_backward width="50%"}
...
:::{.column-margin}
We use the color ${\colorbox{{rgb}{0.2,0.9,0.2}}{\makebox(0,0){}}}$ for data/activations being passed forward through the network.
:::
...
The full algorithm is summarized in algorithm @fig-backpropagation-backprop_for_chains.

![](figures/backpropagation/backprop_for_chains.png){#fig-backpropagation-backprop_for_chains width="100%"}
...
\end{aligned}$${#eq-backpropagation-linear_backward_costgrad} To get an intuition for
...
![The forward and backward matrix multiples for a linear layer.](figures/backpropagation/linear_forward_backward_matrices.png){#fig-backpropagation-linear_forward_backward_matrices width="100%"}
...
![Linear layer forward and backward.](figures/backpropagation/linear_layer_backprop.png){#fig-backpropagation-linear_layer_backprop width="80%"}
...
![Matrix multiply for \texttt{backward} of a pointwise layer.](figures/backpropagation/pointwise_layer_backprop.png){#fig-backpropagation-pointwise_layer_backprop width="90%"}
...
![Matrix multiply for \texttt{backward} of an $L_2$ loss layer.](figures/backpropagation/L2_loss_layer_backprop.png){#fig-backpropagation-L2_loss_layer_backprop width="90%"}
...
![Backward pass through an MLP.](figures/backpropagation/backward_pass_MLP.png){#fig-backpropagation-backward_pass_MLP width="
100%"}
