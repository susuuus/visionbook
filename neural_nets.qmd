...
![A neural network can be drawn as a directed graph.](figures/neural_nets/fig1_net.png){#fig-neural_nets-fig1_net width=50%}
...
![A simple model for a neuron is the perceptron.](figures/neural_nets/perceptron_fig2.png){#fig-neural_nets-perceptron_fig2 width=50%}
...
$$\begin{aligned}    \mathbf{w}^*, b^* = \mathop{\mathrm{arg\,min}}_{\mathbf{w}, b} \frac{1}{N}\sum_{i=1}^N \mathcal{L}(\mathbf{w}^\mathsf{T}\mathbf{x}^{(i)} + b, y^{(i)}):perceptron_learning_problem
...
$$\begin{align}
\mathbb{1}(\alpha < x < \beta) \quad \quad \\
\approx \texttt{relu}\bigg(\frac{x-(\alpha-\gamma)}{\gamma}\bigg) - \texttt{relu}\bigg(\frac{x-\alpha}{\gamma}\bigg) - \texttt{relu}\bigg(\frac{x-(\beta-\gamma)}{\gamma}\bigg) + \texttt{relu}\bigg(\frac{x-\beta)}{\gamma}\bigg)
\end{align}
$${#eq-neural_nets_bump_as_relu_net}

:::{.column-margin}
Here we show how a neural net can represent a function as a sum of basis functions. This idea is also foundational in signal processing, where signals are often represented as a sum of sine waves @sec-fourier_analysis, boxes @fig-upsampling_and_downsampling_nn_interp, or trapezoids @fig-bilinear_interp.
:::
...
![The tensors that represent one pass through the MLP in @fig-neural_nets-simple_MLP_network](figures/neural_nets/simple_MLP_network_tensors_and_batches.png){#fig-neural_nets-simple_MLP_network_tensors_and_batches width=70%}
...
![A 3D tensor that could represent an $C \times H \times W$ color image.](figures/neural_nets/3D_tensor_example-0.png){#fig-neural_nets-3D_tensor_example width=30%}
