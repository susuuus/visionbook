...
infer depth is in @Kaiming2009.
  ...
  ---------------------------------------------------------------------
   ![(a) This image contains multiple 3D cues: shading effects (bump shapes), texture gradients (relative size between bumps), and linear perspective (due to the regular arrangement of the bumps). (b) Aerial perspective. Objects far in the background have lower contrast than objects closer to the camera.](figures/learning_3d/IMG_0508.JPG){width=".53" #fig-aerial_perspective}
                                  \(a\)
  ---------------------------------------------------------------------

  --------------------------------------------------------------------------
   ![](figures/learning_3d/IMG_2772_crop.jpg){width=".53"}
                                    \(b\)
  --------------------------------------------------------------------------
...
intrinsic camera parameters, $\mathbf{K}$, of the sensor.
  ...
  ----------------------------------------------------------------
   ![Image and depth captured by an Azure Kinect sensor. (a) RGB image; and (b) corresponding dense depth map. Both images have the same size of 1280 $\times$ 720 pixels.](figures/learning_3d/rgb.jpg){width=".475" #fig-kinect}
                               \(a\)
  ----------------------------------------------------------------

  ------------------------------------------------------------------------
   ![](figures/learning_3d/depth-turbo.png){width=".475"}
                                   \(b\)
  ------------------------------------------------------------------------
...
![Translating the depth map into a point cloud. This requires a calibrated camera (i.e., knowing the intrinsic camera parameters). The sketch shows how one pixel $(n,m)$ is mapped into a 3D point, $\mathbf{P](figures/learning_3d/from_depthmap_2_pointcloud.png){width="100%" #fig-kinect2}
...
![Typical architecture for depth estimation from a single image\cite{Xian2018](figures/learning_3d/encoder_decoder_arch.png){width="100%" #fig-encoder_decoder_arch_depth}
...
scene from @fig-finding_depth_office in the previous chapter.
  ...
  ---------------------------------------------------------------------------
   ![(a) Office scene. (b) Estimated $Z$ component by the approach from \cite{Ranftl2021,Ranftl2022](figures/learning_3d/IMG_6186_small.jpg){width=".35" #fig-office_midas}
                                     \(a\)
  ---------------------------------------------------------------------------
...
Just looking at the depth map from @fig-office_midasb might seem like
...
that are the average of the values on each side of the discontinuity.
  ...
  -----------------------------------------------------------------------------
   ![Reconstructed point cloud of the office scene with two different view points. Results are good, but not great.](figures/learning_3d/view1_pointcloud.png){width=".44" #fig-office_midas_point_cloud}
                                      \(a\)
  -----------------------------------------------------------------------------
...
![System for unsupervised learning of camera motion and depth \cite{Tinghui2017](figures/learning_3d/unsupervised_depth.png){width="100%" #fig-unsupervised_depth_system}
...
chapters. In section
[\[sect:image_warping\]](#sect:image_warping){reference-type="ref"
reference="sect:image_warping"} we described the procedure for warping
...
reference="chapter-homography"}, if all the videos contain only
