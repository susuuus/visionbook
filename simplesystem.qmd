...
effects, as shown in @fig-parallelProjectionb. Note how, in
@fig-parallelProjectionb, 3D parallel lines in the world are almost
...
$(X,Y,Z)$ projects into the image plane. @fig-projectiona shows our
...
$\theta(x,y)$. @fig-gradient visualizes the edges and the normal vector
on each edge.

<figure id="fig:gradient">
<embed src="figures/simplesystem/gradient.png" style="width:100.0%" />
<embed src="figures/simplesystem/edgeTypes.png" style="width:100.0%" />
...
another.](figures/simplesystem/ground_figure_points-1.png){#fig-ground_figure_points
width=".5"}
...
$$\begin{aligned}
\partial Y / \partial y & = & 1/ \cos(\theta)\end{aligned}$${#eq-derivative_Y_along_edge}
...
\end{aligned}$${#eq-derivative_Y_along_edge_hor} where the vector $\bf t$ denotes direction tangent to
...
](figures/simplesystem/out_of_domain_example-1.png){#fig-out_of_domain
width="100%"}
