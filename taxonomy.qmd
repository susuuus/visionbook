...
shows some pictures taken when illuminating a complex scene with a laser
pointer that illuminate the scene with a narrow beam of light (this is
the best approximation to the image produced by a single light ray that
we could do at home).

![(a)(b)(c)
(a) Scene illuminated with a ceiling lamp. (b-c) Two images
obtained by illuminating a scene with a laser pointer (the red line
indicates the direction of the ray).](figures/taxonomy/lightGray2.png)
...
$\lambda$ (we will study color in chapter
\[[\[chapter:color\]](#chapter:color){reference-type="ref"
reference="chapter:color"}\]) at an instant in time $t$. This function,
...
:::{.column-margin}
**Plenoptic function**: Edward H. Adelson was going to call it
the *holoscopic function*, but a well-known holographer told him that he would punch him in the nose if he called it that.
:::

The plenoptic function does not include information about the observer. The observer
does not have access to the entire plenoptic function, only to a small
slice of it. In we will describe how different mechanisms can produce
images by sampling the ambient light in different ways.
...
scene properties relevant for the observer. shows different images that
illustrate that the human visual system is trying to recover the scenes
that are the cause of those images.
...
](figures/taxonomy/measuringScene2-0.png){#fig-measuringScene
width="100%"}
...
:::{.column-margin}
If you have never read the works of the Greek philosophers, please do. You will be astounded by how much they new about math and physics even though they lacked devices to confirm many of their hypothesis.
:::

How is information about the world being picked up by our eyes? The
Greeks had two competing theories: intromission theories and
extramission (or emission) theories.
...
::: {.column-margin}
![The extramission theory attributed the glow observed in cat eyes to the presence of fire inside them.](figures/taxonomy/cat_eyes.jpg){#fig-cats-eyes}
:::
...
::: {.column-margin}
![Modeling light as traveling along straight lines was a major discovery. This simple model of light sets the foundation upon which image formation, scene geometry, and computer vision rests.](figures/taxonomy/eye_w_fire_aina-1.png)
:::


He listed the following seven axioms of light, extracted from @Burton1945:
...
clarity, is not part of the original figure @Burton1945.](figures/taxonomy/euclid-0.png){#fig-euclid width="60%"}
...
:::{.column-margin}
Credit assignment is unclear. For instance, early versions of the law of reflection are credited to Heron, Ptolemy, Archimedes, Plato, and was potentially known to others before @10.2307/225870.
:::
...
:::{.column-margin}
![](figures/taxonomy/trichromacy_helmholtz-1.png)
:::
...
directions. @fig-helmholtz
shows a schematic illustration of Helmholtz's opthalmoscope,
viewed from above. The glass plates along the diagonal, labeled $a$ in
the figure, allow for viewing the eye under study by the opthalmologist
while reflecting illumination from a light source into the eye. shows a
drawing by Helmholtz of an image from the ophthalmoscope, showing blood
vessels (in background) and branches of the retinal artery and vein over
...
@steinman2000phi. Wertheimer, together with Kurt Koffka and Wolfgang
Köhler, postulated that perceived motion was a new phenomenon that is
not present in the individual stimuli (the two flashing frames) and that
...
Kohler.](figures/taxonomy/gestalt-0.png){#fig-gestalt
width="100%"}
...
Gestalists also studied the problem of **lightness perception**. For a
wonderful book on lightness perception, we refer the reader to
@gilchrist2006, and the work of Edward Adelson @Adelson99. Adelson
defines lightness as "the visual system's attempt to extract reflectance
...
perceptual grouping to explain lightness perception ().
...
the ring are
identical.](figures/taxonomy/koffka_ring-0.png){#fig-koffka_ring
width="100%"}
...
makes the two sides of the ring look different. shows a variant of the
Koffka ring proposed by Adelson @Adelson99 where the grouping cues
create a stronger effect. One can conclude from this experiment that
lightness perception is not a local process and that it involves
considering the context and the principles of grouping.
...
see an induced object appear in front of others. shows several examples
of modal and amodal completion. A beautiful visual illusion that
illustrates modal completion is the **Kanizsa triangle**, shown in . In
this case, we see a triangle that is not really there.
...
the triangle sides.](figures/taxonomy/kanizsa-0.png){#fig-kanizsa
width="100%"}
...
@Gibson1966.](figures/taxonomy/gibson_bird-0.png){#fig-gibson_bird
width="70%"}
...
is very impoverished and that perception is **indirect** as it
has to be complemented by additional processes. The indirect theory of
perception is the most common one. However, the important message to
learn here is that, when building a perceptual system, the richer the
input is, one might hope that the solution to the perception problem
will be simpler.
...
Santiago Ramón y Cajal, using Golgi's stain, isolated individual neurons
and proved that the brain is composed by **networks of interconnected
cells** as shown in his work on the anatomy of the retina
...
Cajal.](figures/taxonomy/cajal-0.png){#fig-cajal width=".6"}

The drawing of the retina () shows the first layers of neurons that
...
applied to record the activity of cells in the cortex. shows some of the
receptive fields found in the LGN. When a cell with an **ON-center
Receptive Field (RF)** is illuminated with a small spot of light
projected on the inner part of the RF, the firing rate increases, as
shown in . The gray rectangle under the graph represents the duration of
the stimulation. When the outer part of the RF is stimulated there is an
inhibition of the cells response. shows an **OFF-center RF**.
...
moment the bar is
visible.](figures/taxonomy/receptive_fields-0.png){#fig-receptivefields
width="100%"}
...
shows an **oriented simple cell**, while shows another type of simple
oriented cell found in the visual cortex. An oriented cell shows
increased firing rate when stimulated with an oriented bar in the
positive region of the RF with the preferred orientation.

shows a schematic of the visual pathways from the retina up to the
primary visual cortex.The first visual area in the visual cortex is
called **V1**. It is a sheet of neurons is arranged along several layers
as shown in . Hubel and Wiesel found that neurons in V1 are organized
along **columns**. When probing with an electrode, as the electrode went
from the surface to the bottom of V1, all the cells found were selective
to the same orientation. When moving the electrode along one horizontal
...
perpendicular horizontal direction, the cells changed the eye
preference, alternating between the two eyes with some binocular cells
between them (ocular dominance columns). A section of around 1 $\times$
3 mm (called a **hypercolumn**) covers all orientations and both eyes
for a small portion of the visual field.
...
from
@kandel:neural.](figures/taxonomy/visual_pathways_aina-0.png){#fig-visual_pathways
width=".7"}
...
Figure recreated from
@Marr82.](figures/taxonomy/necker-0.png){#fig-necker_cube
width=".9"}
...
shows the image of a cube, and its 2.5D sketch as proposed by Marr
@Marr82. The arrows (**gauge-figures**) represent surface orientation.
The dashed lines are surface orientation discontinuities and the black
lines are occlusion boundaries. The pixel intensities represent distance
between the observer and the visible surfaces of the cube. Depth is
represented by gray levels. Intensity increases with depth.

![Image of a cube, and its 2.5D sketch. Figure inspired from figure 4-2
from @Marr82.](figures/taxonomy/sketch-0.png){#fig-sketch
width=".9"}
...
next?](figures/taxonomy/neural_net_enthusiasm-0.png){#fig-taxonomy:neural_net_enthusiam
width="70%"}
...
detect faces in photos @Viola01. This method outperformed
the complicated hand-engineered face detectors of the time in terms of
computational efficiency and detection rate, and even became a popular
algorithm used in consumer cameras.
...
competition @russakovsky2015imagenet. The ImageNet moment was
a watershed for learning-based vision and for neural networks. In the
few years after 2012, learning became the key ingredient in almost all
research on computer vision. As of the 2020s, it is hard to find any
computer vision system that does not use deep learning somewhere in its
pipeline.
