...
Then, the image $\log r(x,y)$ is obtained by integrating the gradients, as shown in @sec-editinggradientdomain, and exponentiating the result. Finally, the illumination can be obtained as $l(x,y) = \text{img}(x,y)/r(x,y)$. The results are shown in Figure @fig-simultaneous3.
