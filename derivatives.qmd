...
![](figures/gaussian_derivative/gaussian_sigma_1.png){#fig-gaussian-derivative-a}  

![](figures/gaussian_derivative/gaussian_x_derivative_sigma_1.png){#fig-gaussian-derivative-b}  
...
As we saw in @chap-simplesystem, an important image representation is given by the image gradient. From the image derivatives, we can also define the image gradient as the vector:
$$
...
:::{#fig-mexican_hat_wavelet}
![](figures/spatial_filters/mexican_hat_wavelet_2d.png){#fig-mexican_hat_wavelet-a width=45%}  
![](figures/spatial_filters/mexican_hat_wavelet_1d.png){#fig-mexican_hat_wavelet-b width=45%}  
...
Magnitude of the DFT of the Gaussian Laplacian with (a) $\sigma=1/2$; (b) $\sigma=1$; (c) $\sigma=2$; and (d) DFT of the five-point discrete approximation, \eqn{\ref{eq-five_point_laplacian}}.
:::
...
:::{#fig-discretelaplacian1d layout-ncol=5}
![](figures/spatial_filters/discretelaplacian1d_a.png){#fig-discretelaplacian1d-a width=20%}  
![](figures/spatial_filters/discretelaplacian1d_b.png){#fig-discretelaplacian1d-b width=20%}  
![](figures/spatial_filters/discretelaplacian1d_c.png){#fig-discretelaplacian1d-c width=20%}  
![](figures/spatial_filters/discretelaplacian1d_d.png){#fig-discretelaplacian1d-d width=20%}  
![](figures/spatial_filters/discretelaplacian1d_e.png){#fig-discretelaplacian1d-e width=20%}  

Comparison between the output of a first-order derivative and the Laplacian of 1D signal. (a) Input signal. (b) Kernel $d_1$. (c) Output of the derivative, that is, convolution of (a) and (b). (d) Discrete approximation of the Laplacian. (e) Output of convolving the signal (a) with the Laplacian kernel (d).
:::

Marr and Hildreth [@Marr80] used zero-crossings of the Laplacian output to compute edges, but this method is not used nowadays for edge detection. Instead, the Laplacian filter is widely used in a number of other image representations. It is used to build image pyramids (multiscale representations, see Chapter @chap-image_pyramids), and to detect points of interest in images (it is the basic operator used to detect keypoints to compute SIFT descriptors [@Lowe04]).
...
Then, the image $\log r(x,y)$ is obtained by integrating the gradients, as shown in section \ref{section-editinggradientdomain}, and exponentiating the result. Finally, the illumination can be obtained as $l(x,y) = \text{img}(x,y)/r(x,y)$. The results are shown in Figure @fig-simultaneous3.
