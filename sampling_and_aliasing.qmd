...
$$
\mathscr{L}_{\delta} (w) = \int_{n=-\infty}^{\infty} \ell_{\delta} (t) \exp{(-jwt)} dt = \int_{n=-\infty}^{\infty} \ell(t) \sum_{n=-\infty}^{\infty} \delta(t-n T_s) \exp{(-jwt)} dt
$$
...
\argmin_{h(t)} \int \left( \mathscr{L}(w) - \mathscr{L}_{\delta} (w) H(w) \right) ^2 dw
$$
...
::: {#fig-aliasingFTzebra}
![Subsampling with aliasing. (a) The zebra sampled with aliasing starts looking like a cow. (b) Fourier transform of the continuous signal $\ell(x,y)$ multiplied by delta trains: $\ell_{\delta](figures/Image_processing_sampling/aliasing_ft_abc1.png){width=100% #fig-aliasingFTzebra}
...
![Subsampling with anti-aliasing filter. (a) Sampled image after processing it with an anti-aliasing filter. (b) Discrete Fourier transform of the corresponding anti-aliased sampled images, $\ell\left[n,m\right]$. Note that now the central part of the Fourier transform is almost not changing.](figures/Image_processing_sampling/aliasing_ft_abc2.png){width=100% #fig-aliasingFTzebra2}
