...
The delta train can be used to model the sampling process. Remember the sampling property of the delta distribution $\ell (t) \delta (t-a) = \ell(a)$. If we multiply a function, $\ell(t)$, by the delta train, $\delta_{T_s}(t)$, we obtain its sampled version at times $nT_s$, which we will denote as $\ell_{\delta} 
...
$$
\capitalimg_s(w) = \sum_{n=-\infty}^{\infty} \ell_s  [ n ] \exp{(-jwn)}
$$

and the continuous Fourier transform of $\ell_{\delta} (t)$, an infinite-length continuous signal, is:

$$
\capitalimg_{\delta} (w) = \int_{n=-\infty}^{\infty} \ell_{\delta} (t) \exp{(-jwt)} dt = \int_{n=-\infty}^{\infty} \ell(t) \sum_{n=-\infty}^{\infty} \delta(t-n T_s) \exp{(-jwt)} dt
$$

$$
= \sum_{n=-\infty}^{\infty} \int_{n=-\infty}^{\infty} \ell(t) \delta(t-n T_s) \exp{(-jwt)} dt = \sum_{n=-\infty}^{\infty} \ell(n T_s) \exp{(-j w n T_s)}
$$
...
$$
\text{sinc} (t/T_s) = \argmin_{h(t)} \int \left( \ell(t) - \ell_{\delta} (
...
The nearest neighbor interpolation is the result of the convolution of $\ell_\delta (t)$ with a box of width $T_s$. In the linear interpolation, the interpolation kernel $h(t)$ is a triangle of width $2T_s$.[^1] Lanczos-1 interpolation consists of using as kernel only the central lobe of the sinc function. This provides an interpolation that is smooth using only a local neighborhood. Other interpolation methods such as cubic interpolation can also be written as convolutions.
...
::: {#fig-aliasingFTzebra}
![Subsampling with aliasing. (a) The zebra sampled with aliasing starts looking like a cow. (b) Fourier transform of the continuous signal $\img(x,y)$ multiplied by delta trains: $\img_{\delta](figures/Image_processing_sampling/aliasing_ft_abc1.png){width=100% #fig-aliasingFTzebra}

Subsampling with aliasing. (a) The zebra sampled with aliasing starts looking like a cow. (b) Fourier transform of the continuous signal $\ell(x,y)$ multiplied by delta trains: $\ell_{\delta}(x,y)$. (c) Discrete Fourier transform of the corresponding sampled signals, $\ell\left[n,m\right]$. (d) Fourier transform of the reconstructed signal with aliasing. (e) Sampled image after processing it with an anti-aliasing filter. (f) Discrete Fourier transform of the corresponding anti-aliased sampled images, $\ell\left[n,m\right]$. Note that now the central part of the Fourier transform is not changing.
:::
...
![Subsampling with anti-aliasing filter. (a) Sampled image after processing it with an anti-aliasing filter. (b) Discrete Fourier transform of the corresponding anti-aliased sampled images, $\img\left[n,m\right]$. Note that now the central part of the Fourier transform is almost not changing.](figures/Image_processing_sampling/aliasing_ft_abc2.png){width=100% #fig-aliasingFTzebra2}

Subsampling with anti-aliasing filter. (a) Sampled image after processing it with an anti-aliasing filter. (b) Discrete Fourier transform of the corresponding anti-aliased sampled images, $\ell\left[n,m\right]$. Note that now the central part of the Fourier transform is almost not changing.
:::
