...
![There are infinite waves (only two shown) that perfectly pass by all the samples.](figures/Image_processing_sampling/sampling_reconstruction1.png)
...
![Delta train with period $T_s=1$. The arrows show when the train's value is infinite. The height of each impulse represents its area.](figures/Image_processing_sampling/delta_train.png)
...
The optimal sampling strategy is the regular hexagonal sampling. This is not the sampling used in computer vision as all images are always represented on a rectangular grid, but hexagonal sampling achieves an increase of around 10