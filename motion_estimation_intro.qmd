...
in @fig-050822_172806\_\_MG_5366.

![Even from a static picture we form a rich representation of the dynamics of the scene. {\em Source](figures/optical_flow/050822_172806__MG_5366.jpg){width=".7" #fig-050822_172806__MG_5366}
...
![Motion-induced visual illusion, after \cite{Murakami2010](figures/optical_flow/moving_bike.png){width="100%" #fig-motion_illusion}
...
Bergen @Adelson85 and briefly discussed in chapter
[\[chapter-filter_banks\]](#sec-filter_banks){reference-type="ref"
reference="chapter-filter_banks"}. In this chapter we will focus on
...
a few moving objects, as shown in @fig-two_frames_from_palma_street.
  ...
  --------------------------------------------------------
   ![Two frames of a video sequence captured from a moving car driving along a busy street in Palma de Mallorca (Spain).](figures/optical_flow/frame1.jpg){width="48%" #fig-two_frames_from_palma_street}
                         (Frame 1)
  --------------------------------------------------------

  --------------------------------------------------------
   ![](figures/optical_flow/frame2.jpg){width="48%"}
                         (Frame 2)
  --------------------------------------------------------
...
$3\times(2s+1)\times(2s+1)$. As in chapter
[\[chapter-perceptual_organization\]](#sec-perceptual_organization){reference-type="ref"
reference="chapter-perceptual_organization"}, we could also use other
...
::: algorithm
[]{#alg:motion_matching_algorithm label="alg:motion_matching_algorithm"}
**Input frames:**
$\boldsymbol\ell_1, \boldsymbol\ell_2 \in \mathbb{R}^{N \times M \times 3}$,
**Output motion flow:**
$\mathbf{u}, \mathbf{v} \in \mathbb{R}^{N \times M}$ **Parameters:**
$L = \text{Maximum displacement}, s = \text{Patch size parameter}$
$\mathbf{r}_1[i,j] = \boldsymbol\ell_1 [i-s:i+s, j-s:j+s] \quad \triangleleft \text{Chop up frame 1 into patches}$
$\mathbf{r}_2[i,j] = \boldsymbol\ell_2 [i-s:i+s, j-s:j+s] \quad \triangleleft \text{Chop up frame 2 into patches}$
$C[i,j][d_i,d_j] = \text{dist}(\mathbf{r}_2[i+di,j+dj], \mathbf{r}_1[i,j])
     \quad \triangleleft \text{Compute matching cost}$
$\mathbf{u}[i,j] = \mathop{\mathrm{arg\,min}}(\min (C[i,j], axis=1)) \quad \triangleleft \text{Estimate  displacement x}$
$\mathbf{v}[i,j] = \mathop{\mathrm{arg\,min}}(\min (C[i,j], axis=0)) \quad \triangleleft \text{Estimate  displacement y}$
:::

Note that padding must be applied if we want to compute the output
optical flow near the image boundaries. Algorithm
[\[alg:motion_matching_algorithm\]](#alg:motion_matching_algorithm){reference-type="ref"
reference="alg:motion_matching_algorithm"} could be written more
...
any grouping cues (as in chapter
[\[chapter-perceptual_organization\]](#sec-perceptual_organization){reference-type="ref"
reference="chapter-perceptual_organization"}) to presegment the image
...
their principles were briefly described in chapter
[\[chapter-filter_banks\]](#sec-filter_banks){reference-type="ref"
reference="chapter-filter_banks"}. Spatiotemporal filtering uses the
...
reference="chapter-sampling"} and
[\[chapter-temporal_filters\]](#sec-temporal_filters){reference-type="ref"
reference="chapter-temporal_filters"}.
...
transform, as shown in @fig-motionIllusion1c. The resulting three
...
Continuing the development of the illusion, @fig-motionIllusion2c shows
...
@fig-motionIllusion1h by applying a periodic sample-and-hold function,
resulting in the spectrum of @fig-motionIllusion1i replicated over
...
@fig-motionIllusion2c.

![Derivation of the fluted square-wave motion illusion, continued from \fig{\ref{fig:motionIllusion1](figures/optical_flow/ted_demo_motion_2.png){width="100%" #fig-motionIllusion2}

Because orientation in space-time tells motion direction (section
[\[sect:modelingSequences\]](#sect:modelingSequences){reference-type="ref"
reference="sect:modelingSequences"}) the space-time plot of
@fig-motionIllusion2b shows that the motion should be perceived to the
left. This will be consistent with the behavior of velocity tuned
filters (section
[\[sect:velocityTunedFilters\]](#sect:velocityTunedFilters){reference-type="ref"
reference="sect:velocityTunedFilters"}) responding to the lowest
spatiotemporal frequency impulses shown in @fig-motionIllusion2c.
However, if we remove the lowest spatial frequency sinusoid from the
signal, the result is shown in @fig-motionIllusion2e, with
spatiotemporal Fourier transform shown in @fig-motionIllusion2g. Now
...
the space-time plot of @fig-motionIllusion2e, and especially if we
apply a low-pass filter, resulting in @fig-motionIllusion2f.
...
components of the signal in @fig-motionIllusion2g, or looking at the
dominant orientations in the space-time plots of
figures [\[fig-motionIllusion2\]](#fig-motionIllusion2){reference-type="ref"
reference="fig-motionIllusion2"}(e and g), would find a signal moving to
