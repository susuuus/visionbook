...
![](./figures/generative_models/graphical_model_z_to_x_white.png){width="30%"}
:::
...
![A simple generative model that draws images of rivers.](./figures/generative_models/simple_rivers_script.png){width="100%" #fig-generative_models-simple_rivers_script}
...
follows: 
$$\begin{aligned}
    p_{\theta}^* &= \mathop{\mathrm{arg\,min}}_{p_\theta} \texttt{KL}\texttt{KL}\KLdivxx{p_{\texttt{data}}}{p_{\theta}}\\
...
| :---: | :---: | :---: | :---: |
| Energy-based models | $x$ | $\checkmark$ (energy only) | $x$ |
| Gaussian | $x$ | $\checkmark$ | $x$ |
| Autoregressive models | $x$ | $\checkmark$ | $\checkmark$ (slow) |
| Diffusion models | $\checkmark$ (high-dimensional) | $x$ | $\checkmark$ (slow) |
| GANs | $\checkmark$ | $x$ | $\checkmark$ |
| VAEs | $\checkmark$ | $x$ | $\checkmark$ |
...
:::{.column-margin}
As a notational convenience, we define here that $p(\mathbf{x}_i \given \mathbf{x}_1, \ldots, \mathbf{x}_{i-1}) = p(\mathbf{x}_1)$ when $i=1$.
:::
...
yet). Algorithm @fig-generative_models-diffusion_model presents these steps
in more formal detail.

![Diffusion Model](./figures/generative_models/diffusion_model.png){width="100%" #fig-generative_models-diffusion_model}

In @fig-generative_models-diffusion_model we train the denoiser
...
![Architecture of a GAN being trained to generate images of flamingos. The synthetic image in this example is generated by BigGAN~\cite{brock2018large](./figures/generative_models/gan_schematic.png){width="100%" #fig-generative_models-generative_models-gan_schematic}
