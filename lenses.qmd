...
in figures [2.1](#fig:pinholes){reference-type="ref"
reference="fig:pinholes"} and
[2.2](#fig:pinholeSize){reference-type="ref"
reference="fig:pinholeSize"}, one can try to let in more light by making
...
image.](figures/imaging/apertures_and_lenses.pdf){#fig:pinholes
width=".9\\linewidth"}
...
## Lensmaker's Formula {#sect:lensmaker}
...
boundary, for the geometry illustrated in , we have
...
<figcaption>(a) Snell’s law describes the bending of light at interfaces
of differing indices of refraction, <span
class="math inline"><em>n</em><sub>1</sub></span> and <span
class="math inline"><em>n</em><sub>2</sub></span>, in terms of the
angles, <span class="math inline"><em>θ</em><sub>1</sub></span> and
<span class="math inline"><em>θ</em><sub>2</sub></span>, relative to the
interface, or surface, normal. (b) Straw below water surface appears
distorted due to refraction at the air/water boundary. Note that (a) and
(b) are not trivially connected!</figcaption>
</figure>
...
glass as $n$, then Snell's law, as shown in , becomes
$$\theta_1 = n \theta_2,$$ for small bending angles $\theta_1$ and
$\theta_2$.

Consider a lens, and two points along its optical axis at a distance $a$
and $b$ from the lens, as shown in . We seek to find a surface shape for
a lens which creates the light paths shown in : light leaving from any
direction at point $a$ will be focused to arrive at point $b$. shows a
view of (a), with angles and distances distorted for clarity of
labeling.

<figure id="fig:lens">

<figcaption>(a) The geometry of a thin lens. (b) Showing the labels of
adding angles referenced in table <a href="#tab:lensmaker"
data-reference-type="ref" data-reference="tab:lensmaker">2.1</a>, used
to describe the conditions for the lens shape, <span
class="math inline"><em>θ</em><sub><em>S</em></sub></span> to give the
desired focusing. Geometry is distorted for visibility.</figcaption>
</figure>
...
we can write simple expressions for the bending angles shown in as a
function of $\theta_S$, the lens surface orientation at height $c$.
Those relations are summarized below in
table [2.1](#tab:lensmaker){reference-type="ref"
reference="tab:lensmaker"}.

The middle row of table [2.1](#tab:lensmaker){reference-type="ref"
reference="tab:lensmaker"} requires explanation. The light ray within
the lens, depicted in , is not necessarily parallel to the optical axis.
...
::: {#tab:lensmaker}
  ...
  : Relations between angles of thin lens example,
:::
::::

[]{#tab:lensmaker label="tab:lensmaker"}

If we start from the relation in
table [2.1](#tab:lensmaker){reference-type="ref"
reference="tab:lensmaker"} for $\theta_4$, and substitute for each angle
...
\label{eq:ts1}$$ This relationship creates the effect that every ray
emanating at a small angle from point $a$ will be focused to point $b$.
shows that the lens surface angle, $\theta_S$, must deviate from flat in
...
surface, such as , curving according to a radius $R$, we have
$\sin(\theta_S) = c/R$. For small angles $\theta_S$, this reduces to
$$\theta_S = \frac{c}{R},
    \label{eq:ts2}$$ where $R$ is the radius of the sphere, which has
the desired property that $\theta_S \propto c$.

![Relation between $R$ and $\theta_S$, in , for a spherical
lens.](figures/imaging/sphereLens3.pdf){#fig:sphereLens width="30%"}

Substituting into the focusing condition, yields the **Lensmaker's
Formula**, $$\frac{1}{a} + \frac{1}{b} = \frac{1}{f},
\label{eq:lensmaker}$$ where the lens **focal length**, $f$ is defined
to be $$f = \frac{R}{2(n-1)}$$

It is straightforward to show, by rotating the lens in through an angle
$\theta_R$ in the above derivation, thus adding $\theta_R$ to $\theta_S$
in table [2.1](#tab:lensmaker){reference-type="ref"
reference="tab:lensmaker"}, that the lensmaker's equation also holds for
light originating off the optical axis. Thus, under the paraxial and
thin lens approximations, the lens focuses light from points on a plane
onto points on a second plane, both perpendicular to the optical axis,
as illustrated in .
...
on-axis point, $P_0$.](figures/imaging/offAxis.pdf){#fig:rotatedLens
width="100%"}

One can also generalize the equation for the case of a lens with
different radii of curvature, $R_1$ and $R_2$ on the front and back
faces of the lens. Defining the left and right surface angles
$\theta_{s_1}$ and $\theta_{s_2}$, respectively, the middle row equation
of table [2.1](#tab:lensmaker){reference-type="ref"
reference="tab:lensmaker"} becomes
$\theta_{s_1} + \theta_{s_2} = \theta_2 + \theta_3$. Then, the equation
substitutions of Table [2.1](#tab:lensmaker){reference-type="ref"
reference="tab:lensmaker"}, combined with $\theta_{S_i} = c / R_i$, for
$i = 1$ and $2$, lead to
...
shows a demonstration of the focusing property for a thin lens. The
...
## Imaging with Lenses {#sect:imagingWithLenses}

Armed with the lensmaker's formula and the observation of , we can
...
bending. Thus a convex thin lens images as a pinhole camera does,
creating a perspective projection.

<figure id="fig:thinLens">

<figcaption>Light travels straight through the center of a thin lens,
where front and back surfaces are parallel. (a) Lens with non-zero
thickness. (b) Idealized thin lens. (c) Rays passing through the center
of the lens behave like rays passing through a pinhole camera, and thus
lenses impose a <span><em>perspective
projection</em></span>.</figcaption>
</figure>

Two points on opposite sides of the lens at distances $a$ and $b$ from
the lens that satisfy are known as conjugate points. Light from one
conjugate point, traveling through the lens, focuses to the other
conjugate point, and vice versa (see ). The rules for tracing light
...
2.  Parallel rays entering the lens all focus to a point at distance $f$
    behind the lens, per . In this case, one of the focal points is at
    infinity.
...
3.  Any ray passing through the center of the thin lens proceeds in a
    ...
    (see ). Thus, lenses, like pinholes, render the world in
...
<figure id="fig:lenspropt">

<figcaption>Showing some conjugate points for a convex thin lens.
</figcaption>
</figure>

Referring to , the two dots are at distance $f$ from the lens. (a) In ,
parallel rays from infinity focus at a distance f from the lens. show
that as a source of light rays moves closer to the lens, they focus
further away on the other side, while shows how rays emanating from a
distance $f$ from the lens are parallel after the lens, that is, they
focus at infinity.

### Depth of Field

Assume that the lens is part of a camera, focusing light from the world
onto the camera's sensor. If a plane outside of the camera, called the
**focal plane**, and the camera's sensor plane are at conjugate
positions, then an object in the focal plane is focussed sharply onto
the sensor plane. All rays from a point on the object's surface will
come into focus at a point in the sensor plane. This will image a point
in the focal plane to a **circle of confusion** at the sensor plane, as
illustrated in .

![Circle of confusion and depth of field. Referring to , if the object
...
respecting the lensmaker's formula (), then the object will form an
...
@Levoy2010.](figures/imaging/circleOfConfusion.eps){#fig:circleOfConfusion
width="70%"}
...
figures [2.10](#fig:circleOfConfusion){reference-type="ref"
reference="fig:circleOfConfusion"} and
[2.11](#fig:DOFformula){reference-type="ref"
reference="fig:DOFformula"}.
...
plane.](figures/imaging/dof.pdf){#fig:DOFformula width="100%"}
...
approximately $C U/f$, again, for $U >> f$. Referring to the top of , by
similar triangles, we have $$\frac{D_1 f}{C U} = 
    \frac{U - D_1}{\frac{f}{N}}$$ Also by similar triangles, we have
$$\frac{D_2 f}{C U} =
    \frac{D_2+U}{\frac{f}{N}}$$ Defining the depth of field, $D$, to be
$D = D_1 + D_2$, and combining terms, we have
$$D = \frac{2 N C U^2 f^2}{f^4 - N^2 C^2 U^2}$$ When the circle of
...
proportion. This effect can be seen in the bottom of , and in .

<figure id="fig:rulers">

<figcaption>Showing the linear relationship between f-number and depth
of field. From the same camera position, a ruler was photographed, in
(a), (b), and (c), using f/2.0, f/4.0, and f/8.0, respectively. Note the
region of sharp focus doubles from (a) to (b), and from (b) to
(c).</figcaption>
</figure>

### Concave Lenses

The lens we designed in was convex. Also of interest are **concave**
...
orientation, $\theta_{Sx} = -\theta_S$ in . By the reflection symmetry
...
appear to be emanating. This is illustrated in for on-axis, and for
off-axis parallel rays.

<figure id="fig:ConcaveLenses">

<figcaption>(a) A convex lens focuses rays to a point. (b) A concave
lens focuses rays to a virtual point. (c) As with convex lenses, a small
shift in the angle of the incoming rays causes a shift in the focus
point at the focal plane.</figcaption>
</figure>
...
illustrated in . The focus point for a set of parallel rays entering a
...
considerations. As shown in , we position a convex lens, lens 1, with a
...
of figures [2.13](#fig:ConcaveLenses){reference-type="ref"
reference="fig:ConcaveLenses"}(a) and
[2.13](#fig:ConcaveLenses){reference-type="ref"
reference="fig:ConcaveLenses"}(b) will cause parallel rays from a
distant object entering lens 1 to become a compressed set of parallel
rays leaving lens 2. Referring to , an angular deviation of the input
rays of $\delta_i$ will become an angular deviation, exiting the convex
lens of $\delta_o$.

<figure id="fig:telescope">

<figcaption>(a) Galilean telescope. The convex lens 1 and the concave
lens 2 share the same focal point, marked with a dot, resulting in
parallel rays in giving parallel rays out. (b) A small change in the
input ray angle gives a larger change in the output ray angle. The
magnification of the telescope is the ratio, <span
class="math inline"><em>M</em></span>, of the angular deviation of an
output ray, <span
class="math inline"><em>δ</em><sub><em>o</em></sub></span>, to the
angular deviation of an input ray, <span
class="math inline"><em>δ</em><sub><em>i</em></sub></span>. Writing the
focal point offset, <span class="math inline"><em>d</em></span>, in
terms of the rays passing through the centers of lenses 1 and 2 gives
the relation, <span
class="math inline"><em>M</em> = <em>f</em><sub>1</sub>/<em>f</em><sub>2</sub></span>,
where <span class="math inline"><em>f</em><sub>1</sub></span> and <span
class="math inline"><em>f</em><sub>2</sub></span> are the respective ray
focal lengths.</figcaption>
</figure>

By property 3 of , and the small angle approximation, we have
...
shows a homemade telescope, inspired by Galileo's. The tube is a 2 in.
...
<figure id="fig:homemade">

<figcaption>Homemade Galilean telescope. (a) The two lenses of the
...
with black paper to reduce stray light.</figcaption>
</figure>

compares images made from this homemade telescope with drawings Galileo
...
<figcaption>(a) Moon as viewed through homemade telescope of Galileo’s
...
his telescope, with 30x magnification.</figcaption>
</figure>
