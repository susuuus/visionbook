# Object Recognition {#chapter-object_recognition}
...
![](figures/object_recognition/what_is_a_chair_with_text.jpg){width="50%"}
...
![](figures/object_recognition/do_it_yourself-1.png){width="50%"}
...
[\[chapter-bias_and_shift\]](#chapter:bias_and_shift){reference-type="ref"
reference="chapter-bias_and_shift"}, studies in human visual perception
...
![](figures/object_recognition/mental_rotation-1.png){width="60%"}
...
::: center
  ...
  --------------- ------------- -------------------
  1-3 Furniture   Chair         Kitchen chair
                                Living-room chair
                  Table         Kitchen table
                                Dining-room table
                  Lamp          Floor lamp
...
reference="sec-intro_to_learning:image_classification"}. Even though we
...
![](figures/object_recognition/image_classification-1.png){width="50%"}
...
![](figures/object_recognition/classification_training_set_v2.png){width="100%"}
...
$$\begin{aligned}
    p(Y_c=1 \bigm | \mathbf{x}) &= \hat{y}_c :classification_probability_model
\end{aligned}$${#eq-object_recognition}
...
$$\sum_{c=1}^K \hat{y}_c=1 :sum_to_one_constraint$${#eq-object_recognition}where $0>\hat{y}_c>1$. In the toy example with three classes shown
...
![](figures/object_recognition/simplex_v2.png){width=".3"}
...
[\[sec-intro_to_learning:image_classification\]](#sec:intro_to_learning:image_classification){reference-type="ref"
reference="sec-intro_to_learning:image_classification"}. Given the
...
$K$ classes: $$\begin{aligned}
    p_{\theta}(Y \bigm | \mathbf{x}) &= \hat{\mathbf{y}} = f_{\theta}(\mathbf{x})
\end{aligned}$$ where $Y$ is the random variable indicating the single
...
![](figures/object_recognition/class_architecture-1.png){width="70%"}
...
$$\prod_{t=1}^T \prod_{c=1}^K \left( \hat{y}^{(t)}_c \right) ^{y^{(t)}_c}:classification_likelihood$${#eq-object_recognition}
...
[\[chapter-convolutional_neural_nets\]](#chapter:convolutional_neural_nets){reference-type="ref"
reference="chapter-convolutional_neural_nets"}, maximizing the
likelihood corresponds to minimizing the **cross-entropy loss** (which
we get by just taking the negative log of the equation
\[[\[eqn:object_recognition:classification_likelihood\]](#eqn:object_recognition:classification_likelihood){reference-type="ref"
reference="eqn:object_recognition:classification_likelihood"}\]),
resulting in a total classification loss:
...
$$\text{TOP-1} = \frac{100}{T}
    \sum_{t=1}^T \mathbb{1}( \hat{y}^{(t)} = y^{(t)})$$ where $y^{(t)}$
is the ground truth class of image $t$. This evaluation metric only
...
::: center
  ...
  ---------------- ----- ----------------- ------ ------ --------------------------
  3-5              Cat          Car         Dog          
  2-5              Cat         $80$         $5$    $15$   $\rightarrow \sum = 100$
  2-5 True class   Car          $2$         $95$   $3$    $\rightarrow \sum = 100$
  2-5              Dog          $4$         $0$    $96$   $\rightarrow \sum = 100$
  2-5                                                    
:::

The elements of the confusion matrix are: $$C_{i,j} = 100 \frac{
...
In this toy example (table
[\[table:confusion_matrix\]](#table:confusion_matrix){reference-type="ref"
reference="table:confusion_matrix"}), we can see that 80 percent of the
...
![](figures/object_recognition/IMG_1260.jpeg){width="32%"}
...
![](figures/object_recognition/pepper.jpg){width="32%"}
...
![](figures/object_recognition/object_detection_bb-1.png){width="40%"}
...
![](figures/object_recognition/scanningWindow2-1.png){width="100%"}

The window scanning algorithm is the following:

::: algorithm
[]{#alg:scanning_window label="alg:scanning_window"} **Input:** Image;
**Output:** Object bounding boxes and classes,
$\{\hat{\mathbf{b}}_i, \hat{\mathbf{y}}_i\}_{i=1}^B$
$\{\hat{\mathbf{b}}_i\}_{i=1}^{B^{\prime}} \leftarrow$ list all bounding
boxes (all locations, scales, aspect ratios, \...)

$\{\hat{\mathbf{b}}_i, \hat{\mathbf{y}}_i\}_{i=1}^B \leftarrow$
Thresholding and nonmaximum suppression []{#alg:object_localization
...
[\[alg:selective_search\]](#alg:selective_search){reference-type="ref"
reference="alg:selective_search"}.

::: algorithm
[]{#alg:selective_search label="alg:selective_search"} **Input:** Image;
**Output:** Object bounding boxes and classes,
$\{\hat{\mathbf{b}}_i, \hat{\mathbf{y}}_i\}_{i=1}^B$
$\{\hat{\mathbf{b}}_i\}_{i=1}^{B^{\prime}} \leftarrow$ Bounding box
proposals
...
![](figures/object_recognition/selective_search-1.png){width="100%"}
...
![](figures/object_recognition/selective_search_architecture-1.png){width=".4"}
...
::: algorithm
[]{#alg:cascade_classifier label="alg:cascade_classifier"} **Input:**
Image, $\mathbf{x}$; **Output:** Object bounding boxes and classes,
$\{\hat{\mathbf{b}}_i, \hat{\mathbf{y}}_i\}_{i=1}^B$

$S_0 = \{\hat{\mathbf{b}}_i\}_{i=1}^{B_0} \leftarrow$ Initial set of
...
![](figures/object_recognition/cascade-1.png){width=".8"}
...
![](figures/object_recognition/detections-1.png){width="100%"}
...
$$\mathcal{L}_{\text{cls}}(\hat{\mathbf{y}}_i, \mathbf{y}_i)
     = -  \sum_{c=1}^{K} y_{c,i} \log(\hat{y}_{c,i})$$ where $K$ is the
number of classes.
...
![](figures/object_recognition/iou-1.png){width="55%"}
...
![](figures/object_recognition/iou_l2_comparison-1.png){width=".9"}
...
$$\mathcal{L}( \{\hat{\mathbf{b}}_i, \hat{\mathbf{y}}_i\}, \{\mathbf{b}_i, \mathbf{y}_i\})
     ...
     \mathcal{L}_{\text{loc}} 
     (\hat{\mathbf{b}}_i, \mathbf{b}_i)$$ where the indicator function
...
![](figures/object_recognition/example_precision_recall-1.png){width=".45"}
...
reference="chap-textures"}). But the distinction between stuff and
...
![](figures/object_recognition/segmenting_trees.png){width="100%"}
...
![](figures/object_recognition/semantic-1.png){width="60%"}
...
![](figures/object_recognition/segmentation_architecture-1.png){width="30%"}
...
in section [1.3](#sec-image_classification){reference-type="ref"
reference="sec-image_classification"} and several architectures are
...
![](figures/object_recognition/instance_segmentation-1.png){width="60%"}
...
![](figures/object_recognition/instance_segmentation_architecture-1.png){width="50%"}
...
$$\mathcal{L}( \{\hat{\mathbf{b}}_i, \hat{\mathbf{y}}_i\}, \{\mathbf{b}_i, \mathbf{y}_i\})
     ...
     \right)$$ The first classification loss corresponds to the
...
![](figures/object_recognition/model_cards_v2.png){width="100%"}
