# Representation Learning {#chapter-representation_learning}
...
![](figures/representation_learning/rep_learning_schematic.png){width=".52"}
...
called a of $\mathbf{x}$. The mapping $f$ is trained so that
...
3.  Compression is an embodiment of *Occam's razor*: among competing
    ...
    sense to which the idea of Bayesian Occam's razor applies (section
    [\[sec-problem_of_generalization:star_problem_revisited\]](#sec:problem_of_generalization:star_problem_revisited){reference-type="ref"
    reference="sec-problem_of_generalization:star_problem_revisited"});
...
principles of compression and prediction in table
[\[tab:representation_learning:types_of_representation_learners\]](#tab:representation_learning:types_of_representation_learners){reference-type="ref"
reference="tab:representation_learning:types_of_representation_learners"}.
...
[]{#tab:representation_learning:types_of_representation_learners
label="tab:representation_learning:types_of_representation_learners"}

## Autoencoders {#sec-representation_learning:autoencoders}

are one of the oldest and most common kinds of representation
...
![](figures/representation_learning/autoencoder_diagram.png){width="100%"}
...
$M < N$, that is, we have performed (although autoencoders can also be
...
$$f^*,g^* = \mathop{\mathrm{arg\,min}}_{f,g} \mathbb{E}_{\mathbf{x}} \left\lVert g(f(\mathbf{x})) - \mathbf{x}\right\rVert_2^2 :autoencoder_learning_problem$${#eq-representation_learning}The idea is to find a lower dimensional representation of the data from
...
PCA projection to $M$-dimensions @bourlard1988auto.### Experiment: Do Autoencoders Learn Useful Representations?
...
![](figures/representation_learning/shapes_dataset_random_samples.png){width="27.5%"}
...
(a) ![](figures/representation_learning/AE_NN_viz.png){width="38%"}
(b)
![](figures/representation_learning/AE_NN_probe.png){width="40%"}
...
We have already seen many kinds of predictive learning, indeed almost
any function can be thought of as making a prediction. In predictive
representation learning, the goal is not to make predictions per se, but
to use prediction as a way to train a useful representation. The way to
do this is first encode the data into an embedding $\mathbf{z}$, then
map from the embedding to your target prediction. This gives a
composition of functions, just like with the autoencoder, that can be
trained with a prediction task and yield a good data encoder. The
prediction task is a for learning good representations.
...
![](figures/representation_learning/predictive_learning_examples.png){width="95%"}
...
is a special case of self-supervised learning, where the prediction
...
![](figures/representation_learning/imputation_examples.png){width="100%"}
...
imputation problems. Above we described how object detectors emerge as a
...
![](figures/representation_learning/clustering_f_diagram.png){width="40%"}
...
:::{.column-margin}
Words, of
course, are given additional structure when used in a language (grammar,
connotations, etc.) beyond just being a set of clusters. The same kind
...
![](figures/representation_learning/contrastive_learning_diagram.png){width="75%"}
...
![](figures/representation_learning/contrastive_learning_colorization.png){width="53%"}
...
We will use the shape dataset described in section
[1.4](#sec-representation_learning:autoencoders){reference-type="ref"
reference="sec-representation_learning:autoencoders"}, and will use the
...
![](figures/representation_learning/align_unif_results_shapes_dataset.png){width="100%"}
