...
[]{#chapter-imaging label="chapter:imaging"}
...
orientation.](figures/imaging/brdf-1.png){#fig-lightSpray width="70%"}

Let an incident light ray be from direction $\mathbf{p}$ and of power,
$\ellin(\lambda)$, as a function of spectral wavelength $\lambda$ ().
...
$$\ellout = F \left( \ellin, \mathbf{n}, \lambda, \mathbf{p}, \mathbf{q} \right)
$$ {#eq-Lambert}
...
$\ellout$, is a function only of the surface orientation relative to the
...
\ellout = F_{L} \left( \ellin (\lambda), \mathbf{n}, \mathbf{p} \right)  = a \ellin(\lambda) \left( \mathbf{n} \cdot \mathbf{p} \right),
\label{eq:lambert}$$ where $a$ is the surface **reflectance**, or
...
$$\ell_{\mbox{Phong spec}} = k_s (\mathbf{r} \cdot \mathbf{q})^\alpha \ellin,$$
...
surface, $\ellout$:
$$\ellout = \int_{\mathbf{p}}  a \ellin(\mathbf{p}) \cos(\mathbf{n} \cdot \mathbf{p}) \mbox{d} \mathbf{p}$$
The intensity, $\ellout$, reflecting off a diffuse wall, thus tells us
very little about the incoming light intensity $\ellin(\mathbf{p})$ from
any given direction $\mathbf{p}$. To learn about $\ellin(\mathbf{p})$,
...
appear.](figures/imaging/no_picture_on_a_wall_aina-1.png){#fig-wallpicture
width="100%"}
...
<embed src="figures/imaging/pinholeBag2.png" />
...
axes.](figures/imaging/pinhole_names2-1.png){#fig-pinhole_names
width=".7"}
...
$(n,m)$.](figures/imaging/pinhole_geometry2-1.png){#fig-pinholeGeometry
width="100%"}

If the distance from the sensing plane to the pinhole is $f$ (see ) then
similar triangles gives us the following relations: $$\begin{aligned}
x & = & f \frac{X}{Z} 
...
Equations
([\[eq:perspctiveProj1\]](#eq:perspctiveProj1){reference-type="ref"
reference="eq:perspctiveProj1"}) and
([\[eq:perspctiveProj\]](#eq:perspctiveProj){reference-type="ref"
reference="eq:perspctiveProj"}) are called the **perspective projection
...
color.](figures/imaging/similar_triangles2-1.png){#fig-pinholeGeometry2
width=".7"}
...
$y = Y$.](figures/imaging/orthogonal_projection-1.png){#fig-orthographics
width=".9"}
...
orthographic.](figures/imaging/straw_camera-1.png){#fig-straw
width="100%"}
