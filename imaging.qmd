...
orientation.](figures/imaging/brdf.eps){#fig:lightSpray width="70%"}

Let an incident light ray be from direction $\mathbf{p}$ and of power,
$\imgin(\lambda)$, as a function of spectral wavelength $\lambda$ ().
...
$\imgout$, is a function only of the surface orientation relative to the
...
contribution, $\img_{\mbox{Phong spec}}$, is:
...
Mathematically, integrating the equation for Lambertian reflections
(equation \[[\[eq:lambert\]](#eq:lambert){reference-type="ref"
reference="eq:lambert"}\]) over all possible incoming light directions
...
The intensity, $\imgout$, reflecting off a diffuse wall, thus tells us
very little about the incoming light intensity $\imgin(\mathbf{p})$ from
any given direction $\mathbf{p}$. To learn about $\imgin(\mathbf{p})$,
...
appear.](figures/imaging/no_picture_on_a_wall_aina.eps){#fig:wallpicture
width="100%"}
...
<embed src="figures/imaging/pinholeBag.pdf" style="width:100.0%" />
<embed src="figures/imaging/pinholeBag2.pdf" />
...
axes.](figures/imaging/pinhole_names2.eps){#fig:pinhole_names
width=".7\\linewidth"}
...
$(n,m)$.](figures/imaging/pinhole_geometry2.eps){#fig:pinholeGeometry
width="100%"}
...
color.](figures/imaging/similar_triangles2.eps){#fig:pinholeGeometry2
width=".7\\linewidth"}
...
location of the pinhole camera. \[-1.2in\]
...
$y = Y$.](figures/imaging/orthogonal_projection.eps){#fig:orthographics
width=".9\\linewidth"}
...
orthographic.](figures/imaging/straw_camera.eps){#fig:straw
width="100%"}
