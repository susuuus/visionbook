...
follows: $$\begin{aligned}
    J &= \mathbb{E}_{\mathbf{z}\sim q_{\psi}(Z \bigm | \mathbf{x})}\Big[\log p_{\theta}(\mathbf{x} \bigm | \mathbf{z}) \Big] - \texttt{KL}\texttt{KL}\KLdivxx{q_{\psi}(Z \bigm | \mathbf{x})}{p_{\mathbf{z}}} \quad\quad\triangleleft\quad\text{VAE objective}:vae_objective_form2
\end{aligned}$${#eq-generative_modeling_meets_representation_learning}
...
![Walking in in two orthogonal directions in BigGAN~\cite{brock2018large](./figures/generative_modeling_and_representation_learning/biggan_latent_walk.png){width="60%" #fig-generative_modeling_and_representation_learning-biggan_latent_walk}
