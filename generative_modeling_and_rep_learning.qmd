...
now fully define the VAE learning problem: $$\begin{aligned}
    ...
    &= \mathop{\mathrm{arg\,max}}_{\theta} \sum_{i=1}^N \log \underbrace{\int_{\mathbf{z}} \overbrace{\mathcal{N}(\mathbf{x}^{(i)}; g^{\mu}_{\theta}(\mathbf{z}), g^{\Sigma}_{\theta}(\mathbf{z}))}^{p_{\theta}(\mathbf{x}^{(i)} \shortgiven \mathbf{z})} \overbrace{\mathcal{N}(\mathbf{z}; \mathbf{0}, \mathbf{I})}^{p_{\mathbf{z}}(\mathbf{z})}d\mathbf{z}}_{p_{\theta}(\mathbf{x}^{(i)})}
...
$$\begin{aligned}    p_{\theta}(\mathbf{x}) &= \int_{\mathbf{z}} p_{\theta}(\mathbf{x} \bigm | \mathbf{z})p_{\mathbf{z}}(\mathbf{z})d\mathbf{z} :vae_likelihood\\
...
$$\begin{aligned}    \theta^* &= \mathop{\mathrm{arg\,max}}_{\theta} \frac{1}{M} \sum_{i=1}^N \log \sum_{j=1}^M \overbrace{\mathcal{N}(\mathbf{x}^{(i)}; g^{\mu}_{\theta}(\mathbf{z}^{(j)}), g^{\Sigma}_{\theta}(\mathbf{z}^{(j)}))}^{p_{\theta}(\mathbf{x}^{(i)} \shortgiven \mathbf{z}^{(j)})}
\end{aligned}$${#eq-generative_modeling_and_representation_learning-vae_learning_problem1} 
...
![To estimate $p_{\theta}$](./figures/generative_modeling_and_representation_learning/vae_importance_sampling1.png){width="50%" #fig-generative_modeling_and_representation_learning-vae_importance_sampling1}
...
\end{aligned}$${#eq-generative_models_and_representation_learning} 
...
![Optimal importance sampling estimates $p_{\theta}$](./figures/generative_modeling_and_representation_learning/vae_importance_sampling2.png){width="50%" #fig-generative_modeling_and_representation_learning-vae_importance_sampling2}
...
![A VAE's encoder, $f_{\psi}$](./figures/generative_modeling_and_representation_learning/VAE_encoder.png){width="55%" #fig-generative_modeling_and_representation_learning-VAE_encoder}
...
$$\begin{aligned}    J_q(\mathbf{x},\psi) &= -\texttt{KL}\texttt{KL}\KLdivxx{q_{\psi}(Z \bigm | \mathbf{x})}{p_{\theta}(Z \bigm | \mathbf{x})}\\
...
follows: $$\begin{aligned}
    J &= \mathbb{E}_{\mathbf{z}\sim q_{\psi}(Z \bigm | \mathbf{x})}\Big[\log p_{\theta}(\mathbf{x} \bigm | \mathbf{z}) \Big] - \texttt{KL}\texttt{KL}\KLdivxx{q_{\psi}(Z \bigm | \mathbf{x})}{p_{\mathbf{z}}} \quad\quad\triangleleft\quad\text{VAE objective}
...
$$\begin{aligned}
    \frac{1}{N}\sum_{i=1}^N \Big( \underbrace{\mathbb{E}_{z\sim q_{\psi}(Z \bigm | x^{(i)})}\Big[ \log p_{\theta}(x^{(i)} \bigm | z) \Big]}_{\text{likelihood term}} - \underbrace{\texttt{KL}\texttt{KL}\KLdivxx{q_{\psi}(Z \bigm | x^{(i)})}{p_{z}}}_{\text{KL term}} \Big)
\end{aligned}$$
...
@kingma2013auto for a derivation): $$\begin{aligned}
    \texttt{KL}\texttt{KL}\KLdivxx{q_{\psi}(Z \bigm | x)}{p_{z}} &= \texttt{KL}\texttt{KL}\KLdivxx{\mathcal{N}(\mu_z,\sigma^2_z)}{\mathcal{N}(0,1)}\\
...
![Training a VAE to model the blue distribution. The Gaussian components spread out to tile both the embedding space and the data space.](./figures/generative_modeling_and_representation_learning/VAE_training_iters.png){width="100%" #fig-generative_modeling_and_representation_learning-VAE_training_iters}
...
elaborate version of the script we saw in @fig-generative_models-simple_rivers_script of @sec-generative_models. The script takes in random
...
river extends (using the same algorithm as in @fig-generative_models-simple_rivers_script.) The VAE has discovered a latent
