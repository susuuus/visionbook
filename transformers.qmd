...
are a recent family of architectures that generalize and expand the
...
a row of the matrix: $$\mathbf{T} =  \begin{bmatrix}
...
neurons/tokens is: $$\begin{aligned}
    ...
    \mathbf{T}_{\texttt{out}}[i,:]&= \sum_{j=1}^N w_{ij} \mathbf{T}_{\texttt{in}}[j,:]\\
     \mathbf{T}_{\texttt{out}}&= \mathbf{W}\mathbf{T}_{\texttt{in}}&\quad\quad \triangleleft \quad \text{linear combination of tokens} :lin_comb_tokens
...
$\mathbf{b} \in \mathbb{R}^{N_2 \times d}$): $$\begin{aligned}    \mathbf{T}_{\texttt{out}}&= \mathbf{W}\mathbf{T}_{\texttt{in}}+ \mathbf{b} \quad\quad &\triangleleft \quad \text{fc layer over tokens}
\end{aligned}$$ 
...
reference="chapter-neural_nets"}. In this case, $F_{\theta}$ has
...
use tokens as the primary nodes, rather than neurons. Token
nets are just like neural nets, alternating between layers that mix
...
$\mathbf{q}^\mathsf{T}\mathbf{k}$. Querying each token in
$\mathbf{T}_{\texttt{in}}$ in this way gives us a vector of
similarities: $$\begin{aligned}
    \mathbf{s} = [s_1, \ldots, s_N]^\mathsf{T}&= [\mathbf{q}_{\texttt{question}}^\mathsf{T}\mathbf{k}_1, \ldots, \mathbf{q}_{\texttt{question}}^\mathsf{T}\mathbf{k}_N]^\mathsf{T}:attention_question_keys
\end{aligned}$${#eq-transformers}
...
$$\begin{aligned}    \mathbf{a} &= \texttt{softmax}(\mathbf{s})\\
    ...
    \end{bmatrix} 
\end{aligned}$$  Figure
[\[fig:transformers:attn_arch1\]](#fig-transformers:attn_arch1){reference-type="ref"
reference="fig:transformers:attn_arch1"} visualizes these steps.
...
:::
 ...
 $$\begin{aligned}
    ...
    \end{bmatrix} 
    = \mathbf{T}_{\texttt{in}}\mathbf{W}_q^\mathsf{T}&\triangleleft \quad\quad \text{query matrix} :query_matrix\\
    \mathbf{K}_{\texttt{in}}&= 
     \begin{bmatrix}
...
Finally, we have the attention equation: $$\begin{aligned}    \mathbf{A} &= f(\mathbf{T}_{\texttt{in}}) = \texttt{softmax}\Big(\frac{\mathbf{Q}_{\texttt{in}}\mathbf{K}_{\texttt{in}}^\mathsf{T}}{\sqrt{m}}\Big) &\triangleleft \quad\quad \text{attention matrix}\\
    \mathbf{T}_{\texttt{out}}&= \mathbf{A}\mathbf{V}_{\texttt{in}}
\end{aligned}$$ where the softmax is taken within each row (i.e., over
...
\end{aligned}$${#eq-transformers} where $v$ is the dimensionality of the value vectors and
...
$\texttt{attn}$, are **permutation equivariant**: $$\begin{aligned}    F_{\theta}(\texttt{permute}(\mathbf{T}_{\texttt{in}})) &= \texttt{permute}(F_{\theta}(\mathbf{T}_{\texttt{in}}))\\
    \texttt{attn}(\texttt{permute}(\mathbf{T}_{\texttt{in}})) &= \texttt{permute}(\texttt{attn}(\mathbf{T}_{\texttt{in}}))
\end{aligned}$$ where $\texttt{permute}$ is a permutation of the order
...
kernel size and stride both equal to $K$: $$\begin{aligned}
&\mathbf{T}[n(N/K)+m,c_{\texttt{2}}] = \nonumber\\
&b[c_{\texttt{2}}] +  \sum_{c_{\texttt{1}}=1}^{C_{\texttt{in}}} \sum_{k_1,k_2=-K}^K w[c_{\texttt{1}},c_{\texttt{2}},k_1,k_2] x_{\texttt{in}}[c_{\texttt{1}},K n-k_1,K m-k_2] \quad \triangleleft \quad \text{(tokenization)} :tokenization_as_conv
\end{aligned}$${#eq-transformers} where, for RGB images,
$\mathbf{x}_{\texttt{in}}\in \mathbb{R}^{3 \times N \times M}$,
...
$$\begin{aligned}    \mathbf{t}_{\texttt{out}} &= \mathbf{W}\mathbf{t}_{\texttt{in}}
\end{aligned}$$ When we apply such a layer to each token in the
sequence, we have: $$\begin{aligned}
...
:::
 ...
 In fact it cannot
be otherwise. If you used larger kernel it would break the permutation
...
architectural structure. We will cover this idea in section
[1.11](#sec-transformers:positional_encodings){reference-type="ref"
...
During training, we will give examples like this: $$\begin{aligned}
    \{\mathbf{t}_1, \ldots, \mathbf{t}_n\} &\rightarrow \mathbf{t}_{n+1} :causal_training_batches\\
    \{\mathbf{t}_1, \ldots, \mathbf{t}_{n-1}\} &\rightarrow \mathbf{t}_n\\
    \{\mathbf{t}_1, \ldots, \mathbf{t}_{n-2}\} &\rightarrow \mathbf{t}_{n-1}
\end{aligned}$${#eq-transformers} and so on. We can make all these predictions at once
...
sinusoidal waves take on at each position: $$\begin{aligned}
    ...
    \mathbf{p} &= \begin{bmatrix}
        ...
        \mathbf{p}_y
    \end{bmatrix}
\end{aligned}$$ where $x$ and $y$ are the coordinates of the token. This
...
![](figures/transformers/affine_layer_comparison.png){width="100%"}
