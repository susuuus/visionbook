...
(@fig-transformers-lin_comb_neurons_vs_tokens) - The general form of these equations for multiple input and output
neurons/tokens is: 
...
![A self-attention layer.](figures/transformers/self_attn_layer.png){#fig-transformers-self_attn_layer width="70%"}
...
![One way self-attention could be used to aggregate information across all patches containing the same object, and thereby arrive at a better representation of the object in $\mathbf{t}_2$, the query patch.](figures/transformers/attention_layer_cartoon.png){#fig-attention_layer_cartoon width="100%"}
...
format of an image for image-to-image tasks (@sec-conditional_generative_models-im2imd). To handle these
...
![Transformers are permutation equivariant. For notational simplicity, we omit layer indices on the token variables here.](figures/transformers/permutation_equivariance.png){#fig-transformers-permutation_equivariance width="100%"}
...
![The query, key, and value projections in transformers can be written either as a convolution or a matrix multiply.](figures/transformers/conv_matmul_equivalence.png){#fig-transformers-conv_matmul_equivalence width="100%"}
...
![Masked attention to make multiple causal predictions at once. Black cells are masked; they are filled with zeros.](figures/transformers/masked_attn_one_matmul.png){width="100%" #fig-transformers-masked_attn_one_matmul}
