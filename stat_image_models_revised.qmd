...
![The space of natural images is just very small part of the space of all possible images. In the case of color images with $32\times32$ pixels, most of the space is filled with images that look like noise.](figures/statistical_image_models/spaces_real_images.png){width=".8" #fig-spaces_real_images}
...
![Perceptual importance of simple histogram manipulations. Figure from \cite{Fleming2001](figures/statistical_image_models/illuminationHistograms.png){width="100%" #fig-illumination}
...
%BUILD SOME CUSTOM MADE IMAGES THAT CORRESPOND TO SOME INTERESTING VARIANTS.  For example: additive, occlusions, scale invariant and plot the measured correlation functions.](figures/statistical_image_models/dead_leaves_squares_and_circles.png){width="100%" #fig-deadleaves}
...
the image.
  ...
  -------------------------------------------------------------------
   ![(a) Photograph of hair.  (b) Random draw of Gaussian image model using covariance matrix fit from (a).  For this unusual image, the model works well, but this is an exception for the Gaussian image model.](figures/statistical_image_models/hair1.png){width="40%" #fig-hair}
                                 \(a\)
  -------------------------------------------------------------------
...
![Best values of $a$, as a function of $r$, that maximize the probability of the 1D image $\left[1,1,1,1,1,a,0,0,0,0,0 \right]$ under the wavelet prior model. The optimal $a$ changes when crossing the $r=1$ value.](figures/statistical_image_models/best_a_s1.png){width=".6" #fig-surface_best_a}
...
![Input/output coring curve for maximum posterior denoising for the example of \fig{\ref{fig-waveletBayes](figures/statistical_image_models/waveletlut.png){width="40%" #fig-bayeslut}
...
image position to be denoised.
  ...
  -------------------------------------------------------------------
   ![Regions of support for the nonlocal means denoising algorithm. 
{\em Source:](figures/statistical_image_models/nlm1a.jpg){width="48%" #fig-nlm1}
                                 \(a\)
  -------------------------------------------------------------------
...
![Nonlocal means denoising algorithm results. (left) Original image without noise. (middle) Noisy image. (right) Denoised image. {\em Source:](figures/statistical_image_models/nlmresult.jpg){width="100%" #fig-nlm2}
...
![The space of natural images is just very small part of the space of all possible images. In the case of color images with $32\times32$ pixels, most of the space is filled with images that look like noise.](figures/statistical_image_models/spaces_real_images_and_models_final.png){width="100%" #fig-spaces_real_images_final}
