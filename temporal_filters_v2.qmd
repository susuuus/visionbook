...
where $f_0(x,y)= f (x,y,0)$ is the image being translated, and $v_x$ and $v_y$ are constants. At time $t$ the image $\text{img}_0(x,y)$ is translated by the vector $(v_x t, v_y t)$ as described by @eq-globaly_moving_image. The pixel value at location $(x=0, y=0)$ at time $t=0$ will appear at time $T$ in location $(x=v_x T, y=v_y T)$.
